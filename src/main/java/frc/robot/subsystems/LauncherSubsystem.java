...
public class LauncherSubsystem extends SubsystemBase {
  ...
  public void periodic() { // this method will be called once per scheduler run
    ...
    if (m_launcherRunning && !m_reverse) {
      m_topMotor.set(Constants.Launcher.kTopPower);
      m_bottomMotor.set(Constants.Launcher.kBottomPower);
    } else if (m_launcherRunning && m_reverse){
      m_topMotor.set(Constants.Launcher.kTopReversePower);
      m_bottomMotor.set(Constants.Launcher.kBottomReversePower);
    } else {
      m_topMotor.set(0.0);
